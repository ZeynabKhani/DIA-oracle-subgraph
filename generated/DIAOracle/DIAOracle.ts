...
export class DIAOracle__getValueResult {
  value0: BigInt;
  value1: BigInt;
...
export class DIAOracle extends ethereum.SmartContract {
  static bind(address: Address): DIAOracle {
    return new DIAOracle("DIAOracle", address);
  }

  getValue(key: string): DIAOracle__getValueResult {
    let result = super.call("getValue", "getValue(string):(uint128,uint128)", [
      ethereum.Value.fromString(key)
    ]);

    return new DIAOracle__getValueResult(
      result[0].toBigInt(),
      result[1].toBigInt()
    );
  }

  try_getValue(key: string): ethereum.CallResult<DIAOracle__getValueResult> {
    ...
    return ethereum.CallResult.fromValue(
      new DIAOracle__getValueResult(value[0].toBigInt(), value[1].toBigInt())
    );
  }
